	...
	if ( 'failed' === $http_request_login ) {
		...
		if ( isset( $http_request_type ) ) {
			...
			$message_types = array(
				...
				'default' => array(
						'message' => __( 'Bei ihrer Anmeldung ist etwas schief gelaufen. Bitte stellen sie sicher, dass ihre Eingaben korrekt sind', 'subway' ),
					),
				'__blank' => array(
						'message' => __( 'Required: Nutzername und Passwort können nicht leer sein.', 'subway' ),
					),
				'__userempty' => array(
						'message' => __( 'Required: Nutzername kann nicht leer sein.', 'subway' ),
					),
				'__passempty' => array(
						'message' => __( 'Required: Passwort kann nicht leer sein.', 'subway' ),
					),
			...
			$error_login_message = '<div id="message" class="error">' . esc_html__( 'Error: Falsche Nutzernamen und Passwort kombination.', 'subway' ) . '</div>';
...
if ( isset( $http_request_logout ) ) {
	$error_login_message = '<div id="message" class="success">' . esc_html__( 'Sie haben sich erfolgreich abgemeldet', 'subway' ) . '</div>';
}

$http_request_redirected = filter_input( INPUT_GET, '_redirected', FILTER_SANITIZE_SPECIAL_CHARS );

if ( isset( $http_request_redirected ) ) {
	$error_login_message = '<div id="message" class="success">' . esc_html__( 'Diese Seite ist nur für Mitglieder. Bitte nutzen Sie den Login um diese Seite zu sehen.', 'subway' ) . '</div>';
}
...
<?php } else { ?>
	<div class="mg-top-35 mg-bottom-35 subway-login-sucessfull" style="background: #CDDC39; padding: 15px 15px 15px 15px;border-radius: 4px;color: #616161;">
		<p style="margin-bottom: 0px;">
			<?php $success_message = apply_filters( 'subway_login_message_success', esc_html__( 'Sie haben sich erfolgreich angemeldet.', 'subway' ) ); ?>
